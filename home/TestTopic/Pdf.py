        ...
        for i, question in enumerate(questions):
            ...
            if selected_options and i in selected_options:
                ...
                if user_answer == correct_answer:
                    ...
                    result_text = f"Incorrect. Correct answer: {correct_option}) {correct_answer}"
...
def generate_all_quiz_pdfs(quiz_data):
    ...
    if hasattr(st.session_state, "quiz_level"):
        ...
        pdf_questions.cell(
            0, 10, f"Difficulty Level: {st.session_state.quiz_level}",
    ...
    if "selected_options" in quiz_data:
        correct_count = sum(
            1 for i, question in enumerate(quiz_data["questions"])
            if quiz_data["selected_options"].get(i) == question["Options"].get(question["Correct_option"])
        )
        ...
        pdf_user_answers.cell(
            0, 10, f"Final Score: {correct_count}/{len(quiz_data['questions'])}",
            new_x="LMARGIN", new_y="NEXT"
        )
...
def generate_answer_key_text(quiz_data):
    ...
    answer_keys = [question["Correct_option"].capitalize() 
